-- How a source file was written, recorded on the row that names it.
--
...
--
-- So the level moves to the row that knows it. After this, `blob.zstd_level` means one
-- thing only -- the level of the legacy content-addressed copy, read only while
-- `file.storage_path` is null -- and `file.zstd_level` is what every reader of a file
-- follows.
--
-- Nullable, like `storage_path` and for the same reason: a row written before this
-- migration by something outside `insert_part_chain` has no level to record, and `NULL`
