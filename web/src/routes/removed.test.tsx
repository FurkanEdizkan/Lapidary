...
function stub(
  parts: PartCard[],
  purge: { quarantined: number; quarantinedBytes: number } = {
    quarantined: 1,
...
test("confirming purge calls the route and reports what is kept, never what is freed", async () => {
  ...
  await waitFor(() =>
    expect(
      calls.some(
        (call) =>
          call.method === "POST" && call.url.includes(`${MOUNTING.id}/purge`),
      ),
    ).toBe(true),
  );
...
test("restore calls the route for the row it was clicked on", async () => {
  ...
  await waitFor(() =>
    expect(
      calls.some(
        (call) =>
          call.method === "POST" && call.url.includes(`${SPARES.id}/restore`),
      ),
    ).toBe(true),
  );
