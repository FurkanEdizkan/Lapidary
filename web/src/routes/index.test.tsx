...
const ok = (body: unknown) => async (): Promise<StubResponse> => ({
  ok: true,
  json: async () => body,
});
...
const WEBP_ORANGE =
  "data:image/webp;base64,UklGRh4AAABXRUJQVlA4TBEAAAAvA0AAAAdQvJoXpf+BiOh/AAA=";
...
const MOTOR_MOUNT: PartCard = {
  ...
  tessellationL0: null,
  sourceHash:
    "33237f7971cb1497a5417c667e9a459c240943c7378b44fcd4f6404590363895",
  sourceBytes: 624_384,
  storedBytes: 197_012,
  compressed: true,
...
const HEX_NUT: PartCard = {
  ...
  tessellationL0: null,
  sourceHash:
    "a0763a33d499b598864ddd26eeca15f6d9794ce44185883fd969888941de365d",
  sourceBytes: 99_284,
  storedBytes: 26_741,
  compressed: true,
...
const SHAFT_COUPLER: PartCard = {
  ...
  tessellationL0: null,
  sourceHash:
    "c6b1d88498005800fb68ccc2f54588d00bbc1603243fcef5ef8f8000d1be2a70",
  sourceBytes: 148_930,
  storedBytes: 148_930,
  compressed: false,
  ...
  vi.stubGlobal(
    ...
    class {
      ...
      constructor(url: string) {
        ...
        Object.defineProperty(this.entry, "onmessage", {
          get: () => this.onmessage,
        });
        Object.defineProperty(this.entry, "onerror", {
          get: () => this.onerror,
        });
      }
      close() {
...
test("renders an actionable message when the server is unreachable", async () => {
  ...
  expect(
    await screen.findByText(
      "Could not reach the server. Check that the api and db services are running.",
    ),
  ).toBeDefined();
});
...
test("renders a card per part with that part own thumbnail bytes inline", async () => {
  ...
  expect(within(nut).getByRole("img").getAttribute("alt")).toBe(
...
test("renders a thumbnailed part and a thumbnail-less part side by side", async () => {
  ...
  const coupler = await screen.findByRole("article", {
    name: SHAFT_COUPLER.name,
  });
  const nut = screen.getByRole("article", { name: HEX_NUT.name });
...
test("says how many parts are loaded so far when the server hands back another cursor", async () => {
  ...
  expect(
    screen.getByRole("button", { name: strings.parts.loadMore }),
  ).toBeDefined();
});
...
test("scrolling to the end asks the server for the page after the cursor", async () => {
  ...
  const fetchMock = vi.fn(async (url: string) => {
    if (url.startsWith("/api/libraries") && url.includes("/parts")) {
      return url.includes("after=")
        ...
        : {
            ok: true,
            json: async () => ({ parts: [MOTOR_MOUNT], next: cursor }),
          };
    }
  ...
  expect(
    fetchMock.mock.calls.some(([url]) =>
      String(url).includes(`after=${cursor}`),
    ),
...
test("stops polling once the batch reports it finished", async () => {
  ...
  await screen.findByText(strings.scan.finished(6, 0), undefined, {
    timeout: 4000,
  });
...
test("refetches the grid as the worker commits parts", async () => {
  ...
  stubFetch({
    ...
    parts: async () => {
      ...
      return {
        ok: true,
        json: async () => (partsCalls === 1 ? page([]) : page([MOTOR_MOUNT])),
      };
    },
    batch: async () => {
      batchCalls += 1;
...
test("a batch whose jobs all settle as rendered refetches the grid exactly once", async () => {
  ...
  stubFetch({
    ...
    parts: async () => {
      ...
      return {
        ok: true,
        json: async () =>
          partsCalls === 1 ? page([SHAFT_COUPLER]) : page([rendered]),
      };
    },
    batch: async () => {
      batchCalls += 1;
      return {
        ok: true,
        json: async () =>
          batchCalls === 1 ? batchStatus({ total: 4, pending: 4 }) : swept,
      };
    },
  });
  ...
  const before = await screen.findByRole("article", {
    name: SHAFT_COUPLER.name,
  });
  ...
  expect(partsInvalidations[0]?.[0]).toEqual({
...
test("a migration whose status stops answering does not report a scan nobody started", async () => {
    ...
    batch: async () => {
      ...
      return calls === 1
        ? {
            ok: true,
            json: async () =>
              batchStatus({ total: 4, pending: 3, running: 1, migrating: 4 }),
          }
        : { ok: false, status: 404 };
    },
  });
  ...
  expect(
    await screen.findByText(strings.migrate.unknown, {}, { timeout: 4000 }),
  ).toBeTruthy();
...
test("the auto-thumbnail toggle shows off for a library the server says is off", async () => {
  ...
  expect(fetchMock).toHaveBeenCalledWith(
    `/api/libraries/${DEFAULT_LIBRARY_ID}`,
  );
...
test("a rejected setting change falls back to what the server said, not to the click", async () => {
  ...
  expect(
    await screen.findByText(strings.library.autoThumbnailFailed),
  ).toBeTruthy();
...
test("the per-card action renders that part, and polls the batch it was handed", async () => {
  ...
  await waitFor(() =>
    expect(fetchMock).toHaveBeenCalledWith(
      `/api/parts/${HEX_NUT.id}/thumbnail`,
      {
        method: "POST",
      },
    ),
  ...
  expect(fetchMock).not.toHaveBeenCalledWith(
    `/api/parts/${MOTOR_MOUNT.id}/thumbnail`,
    {
      method: "POST",
    },
  );
...
test("the scan button starts a scan and watches it as a scan, not as a render", async () => {
  ...
  await waitFor(() =>
    expect(fetchMock).toHaveBeenCalledWith(
      `/api/libraries/${DEFAULT_LIBRARY_ID}/scan`,
      {
        method: "POST",
      },
    ),
...
test("the progress line follows a batch whose total grows after the first poll", async () => {
  ...
  expect(
    await screen.findByText("Scanning — 0 of 3 files.", undefined, {
      timeout: 4000,
    }),
  ).toBeTruthy();
});
...
test("a sweep that finds nothing missing reads as success, not as an error", async () => {
  ...
  expect(fetchMock).toHaveBeenCalledWith(
    `/api/libraries/${DEFAULT_LIBRARY_ID}/thumbnails`,
    {
      method: "POST",
    },
  );
  expect(
    fetchMock.mock.calls.filter(([url]) => String(url).includes("/jobs/")),
  ).toHaveLength(0);
});
...
test("the library totals report both storage classes and the ratio between them", async () => {
  ...
  expect(
    await screen.findByText(
      "Sources 12.5 MB on disk · derivatives 5.7 MB, 45.8% of source.",
    ),
  ).toBeDefined();
  expect(fetchMock).toHaveBeenCalledWith(
    `/api/libraries/${DEFAULT_LIBRARY_ID}/storage`,
  );
});
...
test("a library with no source bytes reports its totals without a ratio", async () => {
  ...
  expect(
    await screen.findByText("Sources 0 B on disk · derivatives 41 kB."),
  ).toBeDefined();
...
test("the progress line moves from the stream, which is what a hidden tab still gets", async () => {
  ...
  expect(streams[0]?.url).toContain(
    "/jobs/01931b6e-0000-7000-8000-0000000b0001/events",
  );
...
const CLIFF_FACE: PartCard & { directory: string } = {
  ...
  tessellationL0: null,
  sourceHash:
    "5b8c1f2e9a47d0c3b6154e88f0a2d97361cc4e5b0f18a7d2946b3e5107cd82af",
...
const OLD_BRACKET: PartCard & { directory: null } = {
  ...
  sourcePath: "brackets/corner-bracket-40x40.stl",
  ...
  tessellationL0: null,
  sourceHash:
    "e41d2b7c05986aa3f0d4b8172c9e5a63d081fb42c7e9503a186dd47b2c9f0e35",
  sourceBytes: 212_004,
  storedBytes: 64_118,
  compressed: true,
...
test("selecting a category puts it in the URL and asks the grid for that category", async () => {
  ...
  // someone can send.
  await waitFor(() =>
    expect(router.state.location.searchStr).toContain(`folderId=${ROCKS.id}`),
  );
  await waitFor(() =>
    expect(fetchMock).toHaveBeenCalledWith(
  ...
  await waitFor(() =>
    expect(router.state.location.searchStr).not.toContain("folderId"),
  );
});
...
test("a model still in the shared store says so rather than showing an invented path", async () => {
  ...
  expect(
    within(card).getByText(strings.folders.directoryPending),
  ).toBeDefined();
  ...
  expect(
    within(card).queryByRole("button", {
      name: strings.folders.moveToFor(OLD_BRACKET.name),
    }),
  ).toBeNull();
