...
1. **Delete** — sets `deleted_at`, hides the part. Nothing touches disk. Reversible
   indefinitely. **Built:** `soft_delete_subtree` writes it and the grid hides those rows.
2. **Purge** — a separate, explicitly worded action, which would decrement `ref_count`.
   **Not built.** No route, no job.
3. **Quarantine** — `ref_count` reaching zero would set `blob.quarantined_at = now()` on
   the existing row, in place — never a move to a `quarantine/` directory. A re-scan or a
   new reference to the same hash would clear the flag: reachable by hash while flagged,
   invisible in UI, restorable. **Not built.** The column is real (`0002_parts.sql`), and
   nothing in the workspace reads or writes it; there is no sweep behind the 30-day hold
   either.

Steps 2 and 3 describe what purge will do when it is built, not what runs today. The
consequence is worth stating plainly, because it is also what the UI promises: deleting a
part frees nothing on disk, and no file is removed by anything a user can press.
