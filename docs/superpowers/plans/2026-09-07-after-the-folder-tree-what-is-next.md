...
| 6 | `file.storage_path` is still nullable | **Decided against, 2026-09-07 — it stays nullable.** The estimate above was wrong twice over. It is seven sites, not four (`download.rs:141,154` and `derive.rs:66-81` also branch on it). And `migrate_storage` — 2,639 lines across two crates and two test files — exists only to fill those nulls, so `NOT NULL` makes it unable to ever find work; `enqueue_migration_if_absent` (`jobs.rs:183`) gates on "no pending job for this library" and **not** on null rows existing, so every worker boot would queue a permanent no-op and deleting the machinery becomes forced rather than optional. That narrows the upgrade path to two hops for any store predating `0009`, since the migration's own guard tells an operator to drain the job on the previous version first. A two-hop upgrade for air-gapped deployments is a real operational cost; seven branches that work and are tested reading as defensive is not. |
