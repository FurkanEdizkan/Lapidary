...
Idempotent, and the one thing to run after cloning. It installs the commit-message hook,
reconciles the plugins below, and materializes this project's skills into `.agents/skills/`.
Every run reports what was already correct, so "already fine" is distinguishable from
"did not look".
...
`AGENTS.md` carries the same rules as `CLAUDE.md` under the filename other tools read. It
is generated by `cargo xtask export-agents-md` and gated in CI, the same way
`web/src/bindings` is — edit `CLAUDE.md`, never `AGENTS.md`.

`.agents/skills/` holds the project's skills for tools that cannot install a Claude Code
plugin. `cargo xtask setup` clones them from the declared marketplaces with plain git, and
the directory is gitignored: it is a materialized cache of another repository, not a copy
this one owns. Committing it would fork the skills, which is what moving them out of
`.claude/skills/` was meant to stop.
