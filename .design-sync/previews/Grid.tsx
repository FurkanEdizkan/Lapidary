/*
  The wall of parts: `Grid` at both densities, the selection bar over it while parts are picked,
  and the skeleton a first page shows. Six real fixture parts; only the bracket has its render,
  the others show the empty well of a part whose preview is not generated yet.
*/
...
function Wall({ density, selected }: { density: 'comfortable' | 'compact'; selected?: string[] }) {
  return (
    <Grid
      parts={PARTS}
      density={density}
      layout="detail"
      hostRoot={null}
      selecting={selected !== undefined}
      selected={new Set(selected ?? []) as never}
      onRender={noop}
      onToggle={noop}
      onSelectAll={noop}
      onOpen={noop}
...
/** Two parts picked: the bar floats over the wall with the overlay shadow, the only shadow in the page. */
